...
public class Mockjs {
    ...
    private static final String[] telFirst = "134,135,136,137,138,139,150,151,152,157,158,159,130,131,132,155,156,133,153,173,177,180,181,189,186,185,176,185,178,182,183,184".split(",");
    ...
    static {
        ...
        scriptEngineManager.registerEngineName("mockRandomJs", new NashornScriptEngineFactory());
        engine = scriptEngineManager.getEngineByName("javascript");
        try (
                InputStream mockJs = Mockjs.class.getClassLoader().getResourceAsStream(MOCK_JS_PATH)
        ) {
            assert mockJs != null;
            try (InputStreamReader reader = new InputStreamReader(mockJs)
            ) {
                engine.eval(reader);
            }
...
//            log.error("执行MockJs错误", e);
    ...
    public static Object mockRandom(String script) throws ScriptException {
        ...
        return engine.eval("Mock." + result);
    }
    ...
    public static String image() throws ScriptException {
        ...
        Object random = mockRandom(format);
        return URLUtil.encode(Convert.toStr(random));
    }
